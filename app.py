...
with st.container():
    user_input = st.text_input(
        "Type your action or message (e.g., 'Attack the dragon')",
        key="user_input"
    )
    if st.button("Send"):
        ...
        if text:
            ...
            st.experimental_rerun()  # refresh app after sending
...
choice_counts = (
    ...
    .value_counts()
    .reset_index()
    .rename(columns={"index": "choice", "input": "count"})
...
sent_counts = (
    ...
    .drop_duplicates(subset=["sentiment"])
)

# Bar chart for choice frequency
